<x-filament-widgets::widget>
    <div class="sm:mt-0">
        <dl class="mt-5 grid divide-gray-200 overflow-hidden rounded-lg bg-white shadow grid-cols-2 divide-x divide-y-0">
            <!-- Earnings -->
            <div class="px-4 py-5 sm:p-6">
                ...
                <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
                    <div class="flex flex-col items-baseline text-xl sm:text-2xl font-semibold text-indigo-600">
                        <div>
                            {{ $stats->formatted['partner_earnings'] }}
                        </div>
                        <div
                            @class(['flex items-center', 'text-[11px]', 'text-green-600' => $stats->difference['partner_earnings_up'], 'text-red-600' => !$stats->difference['partner_earnings_up']])>
                            <div>{{ $stats->formatted['difference']['partner_earnings'] }}</div>

                            @if ($stats->difference['partner_earnings_up'])
