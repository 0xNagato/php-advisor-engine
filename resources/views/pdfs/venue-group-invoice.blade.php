...
<body class="bg-white">
    <div class="min-h-[11in]">
            ...
            <div class="max-w-4xl mx-auto">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-base opacity-90">{{ $venueGroup->name }}</p>
                        <div class="mt-2">
                            ...
                            <div class="text-base font-medium">{{ $dueDate->format('M j, Y') }}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-4xl px-6 py-6 mx-auto">
            <!-- Venue Group Summary Section -->
            <div class="mb-6">
                <h2 class="mb-2 text-lg font-semibold text-gray-800">{{ $venueGroup->name }}</h2>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-200">
                                <th class="px-3 py-2 text-xs font-medium text-left text-gray-600">Venue</th>
                                <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Prime Amount</th>
                                <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Non-Prime Amount
                                </th>
                        ...
                        <tbody>
                            @foreach ($venuesData as $venueData)
                                <tr class="border-b border-gray-100">
                                    <td class="px-3 py-2">
                                        <div class="font-medium">{{ $venueData['venue_name'] }}</div>
                                        <div class="text-xs text-gray-500">
                                            Prime: {{ $venueData['prime_bookings_count'] }} |
                                            Non-Prime: {{ $venueData['non_prime_bookings_count'] }} |
                                            Guests: {{ $venueData['total_guests'] }}
                                        </div>
                                    </td>
                                    <td class="px-3 py-2 font-medium text-center">
                                        {{ money($venueData['prime_total'], $currency) }}
                                    </td>
                                    <td class="px-3 py-2 font-medium text-center">
                                        {{ money($venueData['non_prime_total'], $currency) }}
                                    </td>
                                    <td class="px-3 py-2 font-medium text-center">
                                        {{ money($venueData['total_amount'], $currency) }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                            <tr class="bg-gray-50">
                                <td class="px-4 py-3 font-semibold">Group Total</td>
                                <td class="px-4 py-3 font-semibold text-center">
                                    {{ money($primeTotalAmount, $currency) }}
                                </td>
                                <td class="px-4 py-3 font-semibold text-center">
                                    {{ money($nonPrimeTotalAmount, $currency) }}
                                </td>
                                <td class="px-4 py-3 font-semibold text-center">
                                    {{ money($primeTotalAmount + $nonPrimeTotalAmount, $currency) }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <!-- Per Venue Booking Details -->
            @foreach ($bookingsByVenue as $venueId => $data)
                @php
                    $venue = $data['venue'];
                    $primeBookings = $data['primeBookings'];
                    $nonPrimeBookings = $data['nonPrimeBookings'];
                @endphp

                <div class="mt-12 mb-8">
                    <h2 class="mb-2 text-lg font-semibold text-gray-800">{{ $venue->name }} - Bookings</h2>

                    <!-- Non-Prime Bookings for this venue -->
                    @if ($nonPrimeBookings->isNotEmpty())
                        <div class="mb-6">
                            <h3 class="mb-2 font-semibold text-gray-700 text-md">Non-Prime Bookings</h3>
                            <div class="overflow-x-auto">
                                <table class="w-full">
                                    <thead>
                                        <tr class="border-b border-gray-200">
                                            <th class="px-3 py-2 text-xs font-medium text-left text-gray-600">Date</th>
                                            <th class="px-3 py-2 text-xs font-medium text-left text-gray-600">Reference
                                            </th>
                                            <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Guests
                                            </th>
                                            <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">
                                                Incentive Fee</th>
                                            <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Platform
                                                Fee
                                                ({{ \App\Constants\BookingPercentages::NON_PRIME_PROCESSING_FEE_PERCENTAGE }}%)
                                            </th>
                                            <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Total
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($nonPrimeBookings as $booking)
                                            @php
                                                ...
                                                $incentiveFee =
                                                    $booking->guest_count * ($venue->non_prime_fee_per_head * 100);
                                    ...
                                    <tfoot>
                                        <tr class="bg-gray-50">
                                            ...
                                            <td colspan="3" class="px-4 py-3 font-semibold text-center">Non-Prime
                                                Total:</td>
                                            <td class="px-4 py-3 font-semibold text-center">
                                                @php
                                                    $venueNonPrimeTotal =
                                                        collect($venuesData)->where('venue_id', $venueId)->first()[
                                                            'non_prime_total'
                                                        ] ?? 0;
                                                @endphp
                                                {{ money(abs($venueNonPrimeTotal), $nonPrimeBookings->first()?->currency ?? $currency) }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                    ...
                    <!-- Prime Bookings for this venue -->
                    @if ($primeBookings->isNotEmpty())
                        <div class="mb-6">
                            <h3 class="mb-2 font-semibold text-gray-700 text-md">Prime Bookings</h3>
                            <div class="overflow-x-auto">
                                <table class="w-full">
                                    <thead>
                                        <tr class="border-b border-gray-200">
                                            ...
                                            <th class="px-3 py-2 text-xs font-medium text-center text-gray-600">Amount
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    <tfoot>
                                        <tr class="bg-gray-50">
                                            ...
                                            <td colspan="1" class="px-4 py-3 font-semibold text-center">Prime Total:
                                            </td>
                                            <td class="px-4 py-3 font-semibold text-center">
                                                @php
                                                    $venuePrimeTotal =
                                                ...
                                                @endphp
                                                {{ money($venuePrimeTotal, $primeBookings->first()?->currency ?? $currency) }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                    ...
                    <!-- Venue subtotal -->
                    @php
                        $venueData = collect($venuesData)->where('venue_id', $venueId)->first();
                        $venueTotal = $venueData['total_amount'] ?? 0;
                    @endphp
                    @if ($venueTotal != 0)
                        <div class="px-4 py-3 mb-8 font-semibold text-right text-gray-800 bg-gray-100 rounded">
                            {{ $venue->name }} Subtotal: {{ money($venueTotal, $currency) }}
                        </div>
                    @endif
                </div>
            @endforeach
            ...
            @if ($primeTotalAmount != 0 || $nonPrimeTotalAmount != 0)
                ...
                <div class="p-6 text-right rounded-lg bg-indigo-50">
                    <h3 class="text-xl font-semibold text-indigo-900">
                        @if ($balance > 0)
                            PRIMA owes {{ $venueGroup->name }}:
                            <span class="text-indigo-600">
                                {{ money($balance, $currency) }}
                            </span>
                        @else
                            {{ $venueGroup->name }} owes PRIMA:
                            <span class="text-indigo-600">
                                {{ money(abs($balance), $currency) }}
                            </span>
                        @endif
                    </h3>
                </div>
