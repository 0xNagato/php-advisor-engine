<!-- resources/views/filament/pages/venue/prime-calendar.blade.php -->
    ...
    <form wire:submit.prevent="save">
        <div class="relative -mx-6 overflow-x-auto sm:mx-0">
            <table class="min-w-full table-fixed border border-gray-300 bg-white">
                ...
                <tr>
                    <th
                            class="whitespace-nowrap border border-gray-300 px-2 py-1 text-left text-xs uppercase leading-4 tracking-wider text-gray-600">
                        Time Slot
                    </th>
                ...
                @foreach ($timeSlots[array_key_first($timeSlots)] as $index => $slot)
                    <tr class="{{ $index % 2 === 0 ? 'bg-gray-50' : 'bg-white' }} group">
                        <td
                                class="whitespace-nowrap border border-gray-300 px-2 py-1 text-xs font-semibold group-hover:bg-gray-100">
                            {{ Carbon\Carbon::createFromFormat('H:i:s', $slot['start'])->format('g:ia') }}
                        </td>
                        @foreach ($upcomingDates as $date)
                            <td
                                    class="whitespace-nowrap border border-gray-300 px-2 py-1 text-center group-hover:bg-gray-100">
                                <input type="checkbox"
