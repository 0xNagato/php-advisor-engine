...
class ImportIbizaVenueOnboardings extends Command
{
    ...
    public function handle()
    {
        ...
        $admin = User::query()->whereHas('roles', function (Builder $query) {
        ...
        foreach ($csv as $row) {
            ...
            $venuesByContact[$contactEmail]['venues'][] = [
                ...
                'specialties' => filled($data['Specialties (comma-separated)'])
                    ? array_map('trim', explode(',', $data['Specialties (comma-separated)']))
                    : [],
                'cuisines' => filled($data['Cuisines (comma-separated)'])
        ...
        try {
            foreach ($chunkedVenues as $contactData) {
                ...
                try {
                    ...
                    $existingUser = User::query()->where('email', $contact['email'])->first();
                    if ($existingUser) {
            ...
            if (filled($createdOnboardings)) {
                $this->generateAgreementLinksCSV($createdOnboardings);
            }

            if (blank($failedGroups)) {
                ...
                $this->table(
                    ['Contact Email', 'Error'],
                    collect($failedGroups)->map(fn ($group) => [$group['email'], $group['error']])
                );
