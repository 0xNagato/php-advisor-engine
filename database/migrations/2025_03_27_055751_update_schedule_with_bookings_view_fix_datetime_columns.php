...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("
            ...
            SELECT
                ...
                COALESCE(bs.price_per_head, st.price_per_head, r.non_prime_fee_per_head) AS price_per_head,
                COALESCE(bs.minimum_spend_per_guest, st.minimum_spend_per_guest, 0) AS minimum_spend_per_guest,
                st.party_size,
                dr.date AS booking_date,
                TIMESTAMP(CONCAT(DATE_FORMAT(dr.date, '%Y-%m-%d'), ' ', TIME_FORMAT(st.start_time, '%H:%i:%s'))) AS booking_at,
                TIMESTAMP(CONCAT(DATE_FORMAT(dr.date, '%Y-%m-%d'), ' ', TIME_FORMAT(st.start_time, '%H:%i:%s'))) AS schedule_start,
                TIMESTAMP(CONCAT(DATE_FORMAT(dr.date, '%Y-%m-%d'), ' ', TIME_FORMAT(st.end_time, '%H:%i:%s'))) AS schedule_end,
                CASE
                    WHEN COALESCE(bs.prime_time, st.prime_time) = 1
                        AND r.daily_prime_bookings_cap IS NOT NULL
                        AND COALESCE(db.prime_bookings, 0) >= r.daily_prime_bookings_cap
                    THEN 0
                    WHEN COALESCE(bs.prime_time, st.prime_time) = 0
                        AND r.daily_non_prime_bookings_cap IS NOT NULL
                        AND COALESCE(db.non_prime_bookings, 0) >= r.daily_non_prime_bookings_cap
                    THEN 0
                    ELSE COALESCE(bs.available_tables, st.available_tables) - IFNULL(b.booked_count, 0)
                END AS remaining_tables,
                COALESCE(sp.fee, r.booking_fee) AS effective_fee
            FROM date_range dr
            JOIN schedule_templates st ON LOWER(DAYNAME(dr.date)) = st.day_of_week
            LEFT JOIN base_schedule bs ON bs.venue_id = st.venue_id
            ...
            LEFT JOIN (
                ...
                GROUP BY schedule_template_id, DATE(booking_at)
            ) b ON st.id = b.schedule_template_id AND dr.date = b.booking_date
    ...
    public function down(): void
    {
        ...
        // Restore previous version of the view
        DB::statement("
            CREATE OR REPLACE VIEW schedule_with_bookings AS
            WITH RECURSIVE date_range AS (
                SELECT CURDATE() - INTERVAL 1 DAY AS date
